...
- Section band: `<section class="band band--tint" style="--band-accent: var(--color-topic-aem-ai)" aria-labelledby="h-topics"><div class="container">...</div></section>`. The portfolio's chapter treatment (portfolio/styles.css, `.chapter` and its `::before` and `::after` layers, `.tdiv`, `.year-band`): a full-bleed section whose content stays inside `.container`. Every band spans the viewport, has a hairline top edge in `--color-line`, and vertical padding `--band-pad` (`clamp(var(--space-7), 8vw, var(--space-9))`). Variants, each a modifier on `.band`:
  - `.band--plain`: the page background. The default when no variant is given.
  - `.band--alt`: `--color-bg-alt`, solid.
  - `.band--tint`: the portfolio wash. A `::before` layer (`position: absolute; inset: 0; z-index: -1`, the band is `position: relative; isolation: isolate`) painting `radial-gradient(ellipse 90% 55% at 50% 0%, color-mix(in srgb, var(--band-accent) var(--band-tint-strong), transparent), transparent 70%)` over `linear-gradient(180deg, color-mix(in srgb, var(--band-accent) var(--band-tint-soft), transparent), transparent 30%, transparent 78%, color-mix(in srgb, var(--band-accent) var(--band-tint-edge), transparent))`. `--band-accent` defaults to `--color-accent`; a section sets it to a topic color inline.
  - `.band--inverse`: `--color-inverse-bg` with `--color-inverse-text`, and every component inside resolves its tokens for that ground (links, buttons, chips stay readable). At most one per page.
  - `.band--milestone`: a short centered band with the tint wash, hairlines top and bottom, and a `.band__numeral` in the display face at `--text-display` or larger, for a year marker or a "start here" break.
  - `.band--dots` (optional, combinable): the portfolio's motif layer, an `::after` of tiny `--band-accent` dots (`radial-gradient(circle 1.5px at ...)` tiles) at 0.5 opacity. Decorative only, `pointer-events: none`.
  Tokens: `--band-tint-strong`, `--band-tint-soft`, `--band-tint-edge` are percentages defined per theme in tokens.css (the portfolio's dark values are 7%, 4% and 3%; a light theme may need different values so the wash reads on a light ground without muddying it) plus `--band-pad`. Rules: bands are the only full-bleed elements between the header and the footer; two `.band--alt` never touch; a `.band--tint` accent follows the section's topic, otherwise the page accent; the wash never changes text color, so every text pair in section 5 still meets AA on every band in both themes; adjacent bands must read as different grounds in both themes.
- Divider: `<div class="divider" role="separator" aria-label="Topics"><span class="divider__label">01 / Topics</span></div>`. The portfolio's `.tdiv`: a mono uppercase label at `--text-label` and `--tracking-label` in `--color-text-faint` between two hairlines that fade toward the edges (`linear-gradient(90deg, transparent, var(--color-line-strong), transparent)`, or `--color-line` where that already reads). It sits inside a `.container`, at the top of a band or between two groups inside one band. Without a label it is a single fading hairline.
...
11. `#patterns`: page anatomy (breadcrumb, H1, byline, summary, facts, body), heading rules, content-in-HTML rule, image and alt rules, the theme default rule, and "Section rhythm": which band each section of each template uses and why, with the band rules from section 7 restated. Link to `../../../ai-docs/2026-09-13-seo-geo-redesign/seo-page-blueprint.md` (three levels up from a system folder).
...
Section bands on the templates (added 2026-09-14, from the portfolio page Jack wants the site to keep): every section between the page head and the footer is a `.band`. Home: page head `band--plain`; topics `band--tint` with `--band-accent: var(--color-topic-aem-ai)`; latest posts `band--alt`; featured project `band--tint` with the page accent (and `band--dots` where the identity can carry it); credentials `band--plain` opened by a `.divider`; talks `band--tint` with `--band-accent: var(--color-topic-work)`; CTA `band--inverse` (the `.cta-band` component inside drops its own border and surface). Post: everything through the FAQ stays `band--plain`; related, discuss and author box sit in one `band--alt`. Blog: the start-here card and the counts table sit in a `band--tint` with the page accent; the filter bar and the grid are `band--plain`; the FAQ is `band--alt`. Each system may rebalance which sections take which variant if its identity needs it, but home keeps at least five bands across at least three variants, post and blog at least one non-plain band each, and the rules in section 7 hold.
...
Bands: `.band`, every variant and `.divider` are styled and `color-mix` is used for the tint; home.html has at least five `.band` sections across at least three variants, post.html and blog.html at least one non-plain band each; the docs page has `#c-band` and `#c-divider`. In the browser, at 390, 1440 and 2560 every `.band` on home is as wide as the viewport; in each theme, home shows at least three distinct band grounds (background color plus the `::before` layer); the first heading and paragraph inside every `.band--tint` and `.band--inverse` meet 4.5:1 against the band's effective ground, and every filled button inside a `.band--inverse` keeps a 3:1 ground against the band (for tint, the page background blended with `--band-accent` at `--band-tint-strong`, the worst case).

Contrast: every pair in section 5 passes in both themes. On the docs page, every text node rendered inside a `.story[data-theme="dark"]` frame meets 4.5:1 against its effective background (component tokens that resolve a color must be declared on `:root, [data-theme]`, never on `:root` alone, or a dark frame inherits the already-resolved light value).
